...
4. **A web panel for people** — `MEMGRES_WEB_ENABLED=true` on `memgres-server`: sign in (OpenID Connect providers or an admin token), browse and search memory as a graph or a tree, issue tokens for your own AI clients. Read-only in this release. See [docs/WEB.md](docs/WEB.md) and [docs/OIDC.md](docs/OIDC.md).
...
| `MEMGRES_WEB_ENABLED` | `false` | serve the web panel from `memgres-server` (needs `open` or `managed`). With it: `MEMGRES_PUBLIC_URL`, `MEMGRES_MCP_PUBLIC_URL`, `MEMGRES_OIDC_CONFIG`, `MEMGRES_WEB_SESSION_HOURS` (`12`), `MEMGRES_WEB_COOKIE_SECURE` (`true`). See [docs/WEB.md](docs/WEB.md) |
